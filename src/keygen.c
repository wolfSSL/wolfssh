...
#if !defined(WOLFSSH_NO_MLDSA)
/* Matches the traditional-algorithm condition guarding this function's call
 * sites in MakeCompositeTradKey() below, else it's unreferenced when no
 * traditional algorithm is enabled. */
        ...
        byte* tradPriv)
{
    ...
    if (ops->init == NULL || ops->free == NULL || ops->makeKey == NULL ||
            ops->exportPrivOnly == NULL || ops->exportPub == NULL) {
        return WS_NOT_COMPILED;
    }
