...
static void TestSftpForgedHandleRejected(void)
{
    ...
    const char victimPath[] = "wolfssh_poc_victim.tmp";
    ...
    idx = 0;
    SftpPutU32((word32)(sizeof(ownedPath) - 1), pkt + idx); idx += UINT32_SZ;
    WMEMCPY(pkt + idx, ownedPath, sizeof(ownedPath) - 1);
    idx += (word32)(sizeof(ownedPath) - 1);
...
static void TestSftpHandleNamespaceIsolation(void)
{
    ...
    const char ownedPath[] = "wolfssh_poc_ns.tmp";
...
static void TestSftpCloseFailureRemovesHandle(void)
{
    ...
    byte pkt[256];
    char cwd[WOLFSSH_MAX_FILENAME];
    const char path[] = "wolfssh_closefail.tmp";
