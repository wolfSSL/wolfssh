        ...
        void* tradKey, void* heap, const byte* buf, word32 len, word32* idx)
{
    ...
    if (ret == WS_SUCCESS) {
        byte tradPubCheck[COMPOSITE_MAX_TRAD_PUB_SZ];
        word32 tradPubCheckSz = sizeof(tradPubCheck);

        ret = ops->exportPub(tradKey, tradPubCheck, &tradPubCheckSz);
        if (ret == 0 &&
                (tradPubCheckSz != params.tradPubSz ||
                 WMEMCMP(tradPubCheck, pub + params.mldsaPubSz,
                         params.tradPubSz) != 0)) {
            ret = WS_KEY_FORMAT_E;
        }
    }
...
#else
    byte sigId = ID_RSA_SHA2_512;
#endif
#if !defined(WOLFSSH_NO_RSA_SHA2_256) && !defined(WOLFSSH_NO_RSA_SHA2_512)
    word32 k;

    /* Both hashes are built in, so honor the peer's preference for SHA2-512. */
...
int OsshCertCheckOptions(WS_OsshCert* cert)
{
        ...
        if (ret == WS_SUCCESS) {
            ...
            if (OsshNameEq(name, nameSz, "force-command")) {
                ret = GetStringRef(&cert->forceCommandSz, &cert->forceCommand,
                        data, dataSz, &di);
                /* An empty force-command cannot restrict the session; reject
                 * the certificate rather than fall back to an unrestricted
                 * shell. */
                if (ret == WS_SUCCESS && cert->forceCommandSz == 0) {
                    WLOG(WS_LOG_DEBUG, "OSSH: empty force-command rejected");
                    ret = WS_PARSE_E;
                }
            }
                ...
                /* An empty source-address cannot restrict the peer; reject the
                 * certificate rather than treat it as no restriction. */
                if (ret == WS_SUCCESS && cert->sourceAddressSz == 0) {
                    WLOG(WS_LOG_DEBUG, "OSSH: empty source-address rejected");
                    ret = WS_PARSE_E;
                }
            }
            else {
    ...
    /* Extensions: unknown ones are ignored; any order and duplicates are
     * tolerated. The permit-* names are recognized but not enforced; wolfSSH
     * does not gate pty or forwarding on them. */
    idx = 0;
...
static int OsshVerifyRsa(const WS_OsshCert* cert, void* heap)
{
    ...
    if (ret == WS_SUCCESS) {
        ...
        if (sigTypeSz == 12 && WMEMCMP(sigType, "rsa-sha2-512", 12) == 0) {
        ...
        if (sigTypeSz == 7 && WMEMCMP(sigType, "ssh-rsa", 7) == 0) {
...
static int OsshVerifyEcc(const WS_OsshCert* cert, void* heap)
{
    ...
    switch (NameToId((const char*)cert->caKeyType, cert->caKeyTypeSz)) {
        case ID_ECDSA_SHA2_NISTP256:
            hashType = WC_HASH_TYPE_SHA256;
            curveId = ID_CURVE_NISTP256;
            break;
        case ID_ECDSA_SHA2_NISTP384:
            hashType = WC_HASH_TYPE_SHA384;
            curveId = ID_CURVE_NISTP384;
            break;
        case ID_ECDSA_SHA2_NISTP521:
            hashType = WC_HASH_TYPE_SHA512;
            curveId = ID_CURVE_NISTP521;
            break;
        default:
            ret = WS_INVALID_ALGO_ID;
    }

    /* CA key blob: string type, string curve, string Q. The curve name must
     * match the key type, so digest and curve cannot disagree. */
    if (ret == WS_SUCCESS) {
        idx = cert->caKeyTypeSz + LENGTH_SZ;
        ret = GetStringRef(&curveNameSz, &curveName, cert->caKey,
                cert->caKeySz, &idx);
    }
    if (ret == WS_SUCCESS
            && NameToId((const char*)curveName, curveNameSz) != curveId) {
        WLOG(WS_LOG_DEBUG, "OSSH: CA curve name does not match CA key type");
        ret = WS_INVALID_ALGO_ID;
    }
    if (ret == WS_SUCCESS) {
