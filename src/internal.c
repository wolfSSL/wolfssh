        ...
        WS_UserAuthData_PublicKey* pk, WS_UserAuthData* authData)
{
    ...
    /* The rest of the fields in the signature are already in the buffer.
     * Prefer the on-the-wire signed length (robust when the public key field
     * was replaced, e.g. an OpenSSH certificate); fall back to summing the
     * field sizes. */
    if (ret == WS_SUCCESS) {
            ...
            if (rc == WS_SUCCESS) {
                ...
                if (osshUserKey == NULL) {
                    ...
                    if (hasSig) {
                        ...
                        #ifndef WOLFSSH_NO_RSA
                            case ID_SSH_RSA:
                                sigOk = (sigId == ID_SSH_RSA
                                      || sigId == ID_RSA_SHA2_256
                        ...
                        if (sigOk) {
                            pkTypeId = sigId;
                            /* The per-algorithm verifier matches the signature
                             * algorithm against publicKeyType; for RSA the
                             * signature name (rsa-sha2-*) differs from the base
                             * key type, so use the offered signature name. */
