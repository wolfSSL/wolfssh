        ...
        byte certId, byte* der, word32 derSz, int dynamicType)
{
    ...
    if (destIdx >= WOLFSSH_MAX_PVT_KEYS) {
        ...
        #ifdef WOLFSSH_WINDOWS_CERT_STORE
            /* A file-based certificate is replacing this slot's contents;
             * drop any cert-store state, including the store's certificate
             * DER, so the slot is not mistaken for a cert-store key. */
