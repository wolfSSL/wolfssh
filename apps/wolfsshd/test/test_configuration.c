...
/* Build SSH wire-format public key blob: [uint32 typeLen][type][payload].
 * CheckAuthKeysLine() cross-checks the line's type token against this embedded
 * type field. Returns blob size, or 0 if outSz is too small. */
...
/* Confirms CheckAuthKeysLine's SSH-wire-blob type cross-check recognizes
 * all types, and rejects mismatched type tokens. */
static int test_CheckAuthKeysLineTypes(void)
{
    ...
    /* 64 comfortably covers the longest type name in types[] below. */
    ...
    for (i = 0; i < (word32)(sizeof(types) / sizeof(types[0])); i++) {
        ...
        /* Matching key: a line token agreeing with the blob's embedded
         * type proceeds to key comparison and reports success. */
    ...
    /* A line token naming a different key type than the offered blob must be
     * rejected. Built explicitly rather than reusing the loop's last blob, so
     * this still runs even if types[] compiles to zero entries. */
    ...
    WMEMCPY(lineCopy, line, WSTRLEN(line) + 1);
    rc = CheckAuthKeysLine(lineCopy, (word32)WSTRLEN(lineCopy), blob, blobSz, 0);
    if (rc == WSSHD_AUTH_FAILURE) {
        Log(" PASSED.\n");
    }
    else {
        ...
        word32 maxLineSz)
{
    ...
    if (ret == WS_SUCCESS) {
        ...
        /* Provide a real [uint32 typeLen][type] header for CheckAuthKeysLine's
         * cross-check. The rest is non-repeating filler to catch truncation. */
...
{
    int ret = WS_SUCCESS;
    /* blobALastByte differs from blobA only in the final byte to catch
     * dropped-ConstantCompare mutations. Blobs are full SSH wire-format. */
    static const char keyAStr[] = "wolfssh-auth-key-test-A-AAAAAAA";
    ...
    if (ret == WS_SUCCESS) {
        /* A mismatched line type token is rejected (WSSHD_AUTH_FAILURE), not
         * treated as a hard error, allowing SearchKeysFile to keep scanning. */
        Log("    Testing scenario: mismatched line type is rejected.");
