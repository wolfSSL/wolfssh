...
/* Check that certType is a user certificate and that the CA signature_key
 * type is one of the supported algorithms. The embedded user-key-type vs
 * certificate-algorithm consistency is enforced in OsshCertParse(). Returns
 * WS_SUCCESS when valid. */
WOLFSSH_LOCAL int OsshCertCheckType(const WS_OsshCert* cert);

/* Validate the critical options and extensions, following the OpenSSH
 * convention: both lists must be in ascending name order with no duplicates;
 * any unrecognized CRITICAL option is rejected (fail closed); unrecognized
 * extensions are ignored. Extracts the force-command and source-address
 * critical option values into the certificate. Returns WS_SUCCESS when the
