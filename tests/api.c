...
/* wolfSSH_CTX_UseOsshCert_buffer is an unimplemented stub; wolfsshd relies on
 * this exact return to fall through to the X.509 host-key loader. */
static void test_wolfSSH_OsshCert_usebuffer(void)
{
    WOLFSSH_CTX* ctx;
    byte buf[4] = { 0 };
    ...
    ctx = wolfSSH_CTX_new(WOLFSSH_ENDPOINT_SERVER, NULL);
    AssertNotNull(ctx);
    AssertIntEQ(wolfSSH_CTX_UseOsshCert_buffer(ctx, buf, (word32)sizeof(buf)),
        WS_UNIMPLEMENTED_E);
    wolfSSH_CTX_free(ctx);
}
...
int wolfSSH_ApiTest(int argc, char** argv)
{
    ...
    test_wolfSSH_OsshCert_usebuffer();
#endif
#ifdef WOLFSSH_KEYBOARD_INTERACTIVE
