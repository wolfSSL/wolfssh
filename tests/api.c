...
/* Committed positive test vectors: three OpenSSH user certificates, each over
 * an Ed25519 user key with a "force-command" critical option and signed by a
 * different CA (Ed25519, RSA, ECDSA), made non-expiring (always:forever). They
 * are self-verifying: the signing CA public key is embedded in the cert, so the
 * ossh.c parse and CA-signature verify paths get coverage under make check
 * without ssh-keygen or the --enable-ossh-certs shell tests. */
...
static void test_wolfSSH_OsshCert_valid(void)
{
    ...
    blobs[0] = ossh_vec_ed;  sizes[0] = (word32)sizeof(ossh_vec_ed);
    blobs[1] = ossh_vec_rsa; sizes[1] = (word32)sizeof(ossh_vec_rsa);
    blobs[2] = ossh_vec_ecc; sizes[2] = (word32)sizeof(ossh_vec_ecc);
...
/* OsshCertParse and OsshCertVerifySignature run on the attacker-controlled
 * certificate blob before any CA-trust check, so a malformed/truncated blob
 * from an unauthenticated peer must be rejected with a negative return and
 * must not crash. Run under ASan to confirm no out-of-bounds read and no free
 * of an uninitialized key (the CA verifiers allocate the key before parsing
 * its inner fields). */
static void test_wolfSSH_OsshCert_malformed(void)
{
    ...
    word32 i;
    int t;
    /* CA key type names whose bodies are then truncated. */
    static const char* caTypes[] = {
        "ssh-ed25519", "ssh-rsa", "ecdsa-sha2-nistp256"
    };

    /* Parser: every truncation of a hostile buffer must fail, never succeed
    ...
    /* Verifiers: a CA key blob holding only its type string (no key body)
     * parses far enough to dispatch by CA type, then fails on the truncated
     * body. Exercises the allocate-key-before-parse path for all three. */
    WMEMSET(sig, 0xA5, sizeof(sig));
    for (t = 0; t < (int)(sizeof(caTypes) / sizeof(caTypes[0])); t++) {
...
/* Direct coverage for OsshCertCheckOptions: the ascending-order/no-duplicate
 * rule, unknown-critical-option rejection, malformed option data, and the
 * tolerate-unknown-extension behavior. These branches are otherwise only hit by
 * the --enable-ossh-certs shell test, so cover them under plain make check.
 * SSH strings are a 4-byte big-endian length followed by that many bytes; each
 * option is a name string then a data string, and for force-command/
 * source-address the data string wraps the option value string. */
static void test_wolfSSH_OsshCert_options(void)
{
    ...
    /* permit-pty before permit-agent-forwarding: not ascending. */
